...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rateLimit = checkRateLimit(`experiment-assign:${ip}:${safeSessionId || user?.id || 'anonymous'}`, {
      limit: 60,
      windowMs: 60 * 1000,
    });
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { error: 'Too many assignment requests' },
        { status: 429, headers: rateLimitHeaders(rateLimit) }
      );
    }

    const adminClient = getAdminClient();
    const { data, error } = await adminClient.rpc('get_experiment_assignment', {
